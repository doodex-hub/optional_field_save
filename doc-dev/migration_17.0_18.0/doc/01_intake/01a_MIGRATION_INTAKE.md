...
- Yang sengaja diubah/di-drop selama migrasi: **(update 2026-08-24, Step 6/G2)** satu perubahan disengaja disetujui pemilik modul — `webclient.js` diberi `this.orm = useService("orm")` yang sebelumnya tidak ada, memperbaiki bug KRITIS pre-existing (MF-02) yang membuat webclient blank total setiap login di 17.0 MAUPUN 18.0 (dikonfirmasi identik di kedua versi via eksekusi browser nyata). Ini SATU-SATUNYA penyimpangan dari "port bug-for-bug" di project ini — detail lengkap, bukti eksekusi, dan alasan keputusan di `FINDINGS.md` MF-02. Selain ini, tidak ada perubahan business logic disengaja lain. Perubahan wajib murni kompatibilitas API 18.0 (`getOptionalActiveFields`→`computeOptionalActiveFields`, MF-01) didokumentasikan terpisah di `03_MIGRATION_SPEC.md`.
