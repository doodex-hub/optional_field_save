...
- Yang sengaja diubah/di-drop selama migrasi:
  - **(2026-08-24, Step 6/G2) Perubahan disengaja #1 (MF-02):** `webclient.js` diberi `this.orm = useService("orm")` yang sebelumnya tidak ada, memperbaiki bug KRITIS pre-existing yang membuat webclient blank total setiap login di 17.0 MAUPUN 18.0 (dikonfirmasi identik di kedua versi via eksekusi browser nyata). Detail: `FINDINGS.md` MF-02.
  - **(2026-08-26, Step 9) Perubahan wajib kompatibilitas (MF-05, BUKAN judgment call preserve-vs-fix — analog MF-01):** `webclient.js` dan `list_renderer.js` diubah dari `session.partner_id` jadi `user.partnerId` (import `@web/core/user`) — `session.partner_id`/`session.uid` DIHAPUS TOTAL dari objek session di Odoo 18.0 (dipindah ke service baru, dikonfirmasi TIDAK ADA di 17.0 — genuine version-diff, bukan bug lama). Tanpa fix ini mekanisme inti modul (load+simpan preferensi) rusak total di 18.0. Detail: `FINDINGS.md` MF-05.
  - Selain dua hal di atas, tidak ada perubahan business logic disengaja lain. Perubahan wajib murni kompatibilitas API 18.0 lainnya (`getOptionalActiveFields`→`computeOptionalActiveFields`, MF-01) didokumentasikan terpisah di `03_MIGRATION_SPEC.md`.
