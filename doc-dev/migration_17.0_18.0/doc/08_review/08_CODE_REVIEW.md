...
| CR-03 | 🔵 Info | Business Logic (edge case) | `list_renderer.js` | 50 | `setDatabase()`: `datapartnerId[0].optional_field_save` diakses SEBELUM cek `datapartnerId.length > 0` (baris 56) — kalau `search_read` kebetulan return array kosong, `datapartnerId[0]` adalah `undefined`, akses `.optional_field_save` akan throw `TypeError`. **Pre-existing dari source 17.0**, secara praktis nyaris tidak pernah ter-trigger (partner user login hampir selalu ada), dan tertangkap `try/catch` di baris 45/75 sehingga tidak crash aplikasi (beda dari MF-02 yang tidak ada try/catch). Serupa pola F-10 (gagal silent, `console.error` saja). | Tidak perlu difix — pre-existing, tertangkap try/catch, di luar scope. Dicatat di sini untuk kelengkapan audit, bukan alasan menolak gate |
